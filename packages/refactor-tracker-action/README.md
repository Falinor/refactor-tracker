...
GitHub Action wrapper for [`refactor-tracker`](https://www.npmjs.com/package/refactor-tracker). Runs detections in CI, persists the cache and state between runs, and posts a sticky PR comment with the delta.
...
- The cache (`.refactor-tracker-cache.json`) and state (`.refactor-tracker-state.json`) files are restored from the default branch's last successful run, so PR deltas are measured against `main` — not against the previous PR push.
- On `push` to the default branch, both files are saved for future runs.
...
| -------------------- | ----------------------- | --------------------------------------------------------------------------------------------- |
| `config-path`        | `.refactor-tracker.yml` | Path to the refactor-tracker config, relative to `working-directory`.                         |
...
| `cache-strategy`     | `actions-cache`         | How to persist the cache and state between runs. One of: `actions-cache` \| `none`.           |
...
- **PR runs** restore the cache and state that the most recent `main` build produced, so deltas are measured against `main`.
- **`push` to default branch** saves both files for future runs.
...
Set `cache-strategy: none` to opt out entirely (e.g. if you wire `actions/cache@v4` yourself or pull the baseline from another source).
...
```yaml
- run: npm install -g refactor-tracker@0.7.0
- uses: Falinor/refactor-tracker/packages/refactor-tracker-action@v1
```

`refactor-tracker@0.7.0` (or later) is recommended — it's the first version to persist the `.refactor-tracker-state.json` file this action restores alongside the cache. The `--report-output` flag the action relies on has been available since `0.4.0`.
